...
public class Autonomous
{
	...
	Autonomous(Robot robot)
	{
		...
		gearPickup = new Gear(robot, null);
		
		vision = Vision.getInstance(robot);
		
		shooter = new BallControl(robot);
	...
	public void execute()
	{
		...
		switch (program)
		{
			...
			case 2:		// Place gear center start.
				placeGearCenter(5600, false, false);
			...
			case 9:		// Place gear center start with vision.
				placeGearCenter(5800, true, false);
	...
	private void placeGearCenter(int encoderCounts, boolean useVision, boolean time)
	{
		gearBox.lowGear();
		...
		if (useVision)
			autoDriveVision(-.45, encoderCounts, true);
		else
...
else
{
	...
	private void placeGearFromSide(boolean leftSide, boolean useVision)
	{
		...
		placeGearCenter(5300, useVision, false);
...
//		}
	...
	private void autoDrive(double power, int encoderCounts, boolean enableBrakes)
	{
		...
		while (runSafe() && Math.abs(encoder.get()) < encoderCounts) 
		{
	...
	private void autoRotate(double power, int angle)
	{
		...
		while (runSafe() && Math.abs((int) robot.navx.getYaw()) < angle) {Timer.delay(.020);} 
		...
		while (driving)
		{
			...
			if (distance != 0)
			{
				if (distance > 150 && distance <= prevDistance)
					...
					driving = robot.isEnabled() && distance < 190;
			...
			{
				driving = runSafe() && Math.abs(encoder.get()) < encoderCounts;
